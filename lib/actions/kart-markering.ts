...
export async function settMarkering(
  ...
  //
  // logg.warn før koersjonen (#767): migrasjon 152 bytter constrainten fra en
  // verdiliste til et format-check, så en klient som sender noe UTENFOR
  // registeret ikke lenger nødvendigvis får en 23514 fra databasen — den kan
  // se helt gyldig ut for constrainten og likevel bli koersert her, stille.
  // Uten dette signalet mister vi muligheten til å oppdage en utdatert klient
  // (cachet bundle med et fjernet symbol) eller et fabrikkert POST.
  if (!erGyldigSymbol(symbol)) {
    logg.warn('kart.symbol.ukjent', { sample: symbol })
  }
