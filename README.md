...
- **Styling via inline `style={{...}}` med CSS-variabler — vurdert og bevisst beholdt.** Komponenter bruker tokens (`var(--accent)` osv), ikke hardkodede verdier. Migrering til CSS-moduler/Tailwind ble formelt vurdert (2026-07) og avvist: appen er mobil-først for touch, så hover/pseudo-klasse-friksjonen er marginal i praksis, og smale inline-diffs passer AI-arbeidsflyten — mot 20+ timers migrering av ~1300 style-objekter med visuell regresjonsrisiko. Tailwind-utilities brukes der de passer (hybrid). Ikke skalerbart for større team, men riktig for dette.
