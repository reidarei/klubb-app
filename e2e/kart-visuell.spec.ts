...
import { MARKERING_SYMBOLER, STANDARD_SYMBOL } from '../lib/markering-symboler'
...
// (lib/klubb-symboler.ts), aldri av klubbens id-er — samme mønster som
// e2e/kart-markering.spec.ts (#767-review). ANNET_SYMBOL er det første som
// IKKE er standard; et register kan lovlig ha bare ett symbol.
...
test.describe('Kartsiden — visuell baseline', () => {
  ...
  test('symbolvelgeren — dark', async ({ page }) => {
    ...
    await bekreft.click()
    await page.getByTestId(`symbol-${ANNET_SYMBOL?.id ?? STANDARD_SYMBOL}`).click()
    await page.waitForTimeout(400)
