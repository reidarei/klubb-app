...
export default async function Innstillinger() {
  ...
  return (
    <div style={{ padding: '0 20px 20px' }}>
      ...
      {(() => {
        // Migrasjon 152 (#767) slutter å seede varsel_innstillinger for
        // symbol-avledede varseltyper (fra SYMBOLER_VARSLER, se lib/klubb-symboler.ts) — settet
        // er klubb-konfigurerbart, og en statisk migrasjon kan ikke forutse et
        // symbol lagt til etter at den ble skrevet. Uten fletten under ville
        // bryteren for et slikt symbol rett og slett forsvunnet fra panelet i
        // det raden mangler — .map() ser aldri en type uten rad. aktiv: true
        // speiler fallback-verdien i erVarselAktiv() (lib/varsler.ts): ingen
        // rad betyr aktiv, ikke av, så den syntetiske raden lyver ikke om
        // faktisk sendeatferd.
        const kjenteNoekler = new Set((innstillinger ?? []).map(i => i.noekkel))
        const syntetiske = SYMBOLER_VARSLER.filter(s => !kjenteNoekler.has(s.varsel.type)).map(
          s => ({ noekkel: s.varsel.type, aktiv: true, beskrivelse: null }),
        )
        const sortert = [...(innstillinger ?? []), ...syntetiske].sort((a, b) => {
      ...
      {/* Funksjoner — app-vide på/av-flagg.
          ...
          fanene: reisemodus og møtemodus er globale kill-switcher og er av
          for alle, også admin, når bryteren står av (#723-review, #780). */}
      <InnstillingsKort
        tittel="Funksjoner"
        oppsummering={[
      ...
      >
        ...
        />
        {/* Kill-switch (#780) — landes AV i migrasjon 153, samme begrunnelse
            som REISEMODUS over. */}
        <FunksjonToggle
          noekkel={MOETEMODUS}
