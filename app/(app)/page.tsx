...
export default async function Forside() {
  // «/» er kartmodusens forside mens en tur eller et møte pågår (#723/#780)
  // — omdirigeres server-side FØR resten av agenda-spørringene kjøres, ikke
  // etterpå: hentKartmodus() er cache()-wrappet og delt med AppLayout, så
  // dette koster ingenting ekstra, men agenda-dataene ville vært bortkastet
  // arbeid på de dagene modusen faktisk pågår.
  const [user, supabase, kartmodus] = await Promise.all([
    getInnloggetBruker(),
    createServerClient(),
    hentKartmodus(),
  ])
  if (kartmodus.paa) redirect('/kart')
  ...
  // Turperioder (start + evt. slutt) så kalenderen kan tegne fly-strek-fly for
  // hele turen, ikke bare avreisedagen (#770). arrangementDatoer utvides IKKE
  // med mellomdager — byggTurMarkering i MiniKalender løser strek-tegningen.
  const turPerioder = arrangementerBerikt
    .filter(a => a.type === 'tur')
    .map(a => ({
      start: norskDatoNokkel(a.start_tidspunkt),
      slutt: a.slutt_tidspunkt ? norskDatoNokkel(a.slutt_tidspunkt) : null,
    }))
