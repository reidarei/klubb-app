...
**Et brukerinnspill leveres eller avslås — det finnes ikke noe midt imellom.** De aller fleste issues i et slikt repo er egne saker fra vanlig utvikling, forvaltning og drift; de har ingen avsender og varsler ingen. Men et issue som faktisk kom fra et medlem skal **enten** leveres (i en eller annen form) og kommenteres på, **eller** avslås som `not planned`. Lukkes et slikt innspill som gjennomført uten en merket endringslogg-oppføring, er kontrakten brutt — enten ble merkelappen glemt, eller issuet ble lukket før endringen var ute, eller det burde vært avslått. Webhooken logger derfor `github.webhook.innspill.uten_endringslogg` som **feil**, ikke advarsel: det skal vekke noen. Standardteksten medlemmet får i det tilfellet er en nødløsning som ikke later som alt er i orden — den skal aldri være i normal bruk.
